...
object RegexParser extends RegexParsers {
  ...
  def concat: Parser[Term] = rep(midPriority) ^^ (list => listToConcat(list))
  def or: Parser[Term] = (highPriority ~ "|" ~ maxPriority) ^^ { case l ~ "|" ~ r => Or(l, r) }
  ...
  def maxPriority: Parser[Term] = or | highPriority


  def listToConcat(value: List[Term]): Term = value match {
    case head :: Nil => head
    case head :: tail => Concat(head, listToConcat(tail))
  }
